...
- `test/ui-test.mjs` — smoke de UI en Chromium headless, viewport iPhone (**47 checks**; deja `test/shot-*.png`. Ver `TASKS.md` para el backlog de features y lo ya hecho. Ojo: los botones de la pestaña Claude disparan en pointerup con slop —`onTap()` en `app.js`, tarea 12—, así que simular un tap requiere el par pointerdown+pointerup.)
