...
| `POST /api/dispatch` | Despacha un agente: crea una sesión tmux en un dir de `WORKSPACES_ROOT` y lanza `claude` con un prompt inicial + `--permission-mode`. Body JSON: `{ "dir": "<basename>", "prompt": "...", "mode": "plan\|acceptEdits\|bypassPermissions" }`. El prompt viaja como argv posicional (quoting ANSI-C `$'...'`, verbatim). 409 si el dir ya tiene sesión (nunca `<nombre>-2`) |
