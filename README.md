...
| `GET /api/host/status` | Salud de la Mac anfitriona: `{ name, battery: { pct, state } \| null, ac, sleepDisabled, uptime, alert, pushMissed }` (parseado de `pmset`/`scutil`; en un desktop sin batería, `battery: null`). `pushMissed` (`{ count, last } \| null`) cuenta las Web Push que no entregaron a nadie — alimenta el banner de suscripción caída |
